...
public class TriangulatedMesh extends Shape{
	...
	@Override
	public Object clone() {
	    TriangulatedMesh tri = null;
	    try {
	    	tri = (TriangulatedMesh) super.clone();
	    } catch(CloneNotSupportedException cnse) {
	      	System.err.println (System.err+"/"+cnse.getMessage ());
	    }
	    
	    tri.culling = culling;
	    tri.normals = new Vector3d[normals.length];
	    for (int i = 0 ; i < normals.length ; i++)
	    	tri.normals[i] = new Vector3d (normals[i]);
	    tri.paths = new int[paths.length][3];
	    for (int i = 0 ; i < paths.length ; i++)
	    	for (int j = 0 ; j < 3 ; j++)
	    		tri.paths[i][j] = paths[i][j];
	    tri.points = new Point3d[points.length];
	    for (int i = 0 ; i < points.length ; i++)
	    	tri.points[i] = new Point3d(points[i]);
