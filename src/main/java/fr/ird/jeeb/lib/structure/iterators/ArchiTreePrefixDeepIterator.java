 ...
 *
 */
public class ArchiTreePrefixDeepIterator {
	...
	public ArchiTreePrefixDeepIterator (ArchiNode startNode) {
	...
	public ArchiNode next() throws NoSuchElementException {
		 if (hasNext()) {
			...
			try {
				if(c != null) {
					Collection<ArchiNode> ln = c.getForwardLinkedNodes(true, false, true);					
				
					if(ln != null && !ln.isEmpty())
						fifo.addAll (0, ln);
					
					List<ArchiNode> ramifications = c.getLinkedRamifications ();
					if(ramifications != null && !ramifications.isEmpty ())					
						fifo.addAll (ramifications);
