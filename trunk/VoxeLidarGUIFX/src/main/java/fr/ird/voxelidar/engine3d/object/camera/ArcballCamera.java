...
public class ArcballCamera extends Camera{
    ...
    /*
    private Vec3F mapToSphere(Point3F newPoint){
        
        Vec3F newVec = new Vec3F();
        newPoint.x = (newPoint.x*1366) - 1.0f;
        newPoint.y = (newPoint.y*768);
        
        float length = (newPoint.x*newPoint.x) + (newPoint.y*newPoint.y);
        
        if(length > 1.0f){
            float norm = (float) (1.0f /Math.sqrt(length));
            
            newVec.x = newPoint.x * norm;
            newVec.y = newPoint.y * norm;
            newVec.z = 0.0f;
    ...
    /*
    public void drag(Point3F newPoint, Quat newRot){
        
        Vec3F enVec = mapToSphere(newPoint);
        
        if(newRot != null){
            
            Vec3F perp = Vec3F.cross(perp, enVec);
        }
    }*/
