...
import fr.amap.lidar.amapvox.jeeb.workspace.sunrapp.geometry.CoordinatesConversion;
import fr.amap.lidar.amapvox.jeeb.workspace.sunrapp.util.Circle;
...
/**
 * Sun position
 * and splitting of direct radiation in turtle sectors // TODO move in turtle methods
 * 
...
public class Sun {
	...
	public boolean position (float latitudeRadian, int doy, float hourDecimal) {
		...
		direction = CoordinatesConversion.polarToCartesian (zenith, azimuth);
		if (zn > Math.PI / 2.) return false;
		return true;
	}
	...
	public static float[] directInTurtle (float direct, Vector3f sunDirection, Turtle turtle) { // TODO to be moved in Turtle class
		...
		if (sunDirection.z < 0) return weights;
