...
public class TmpVoxelAnalysis extends Process implements Cancellable, IVoxelAnalysis{
    ...
    private static List<Point3i> indicesList = new ArrayList<>();
    private static List<Double> opticalPathLength = new ArrayList<>();
    private static List<Float> enteringList = new ArrayList<>();
    private static List<Float> interceptedList = new ArrayList<>();
    ...
    private VoxelAnalysis.LaserSpecification laserSpec;
    ...
    public TmpVoxelAnalysis(Raster terrain, List<PointcloudFilter> pointcloudFilters, VoxelAnalysisCfg cfg) {
    ...
    private void propagate(Point3d origin, Point3d echo, double beamFraction, double residualEnergy, boolean lastEcho, int shotID, Shot shot, int echoID) {
        ...
        while ((context != null) && (context.indices != null)) {
            ...
            if(test && (transMode == 2 || transMode == 3)){
                ...
                indicesList.add(new Point3i(vox.$i, vox.$j, vox.$k));
                opticalPathLength.add(longueur);
                enteringList.add((float)entering);
                interceptedList.add((float)intercepted);
            }
...
//    }
    ...
    public void writeLists() throws FileNotFoundException, Exception{
        
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(new File(outputFile.getAbsolutePath())+".raytracing.txt"))) {

            writer.write("i j k entering intercepted path_length\n");
            for(int i=0;i<enteringList.size();i++){
                writer.write(indicesList.get(i).x+" "+indicesList.get(i).y+" "+indicesList.get(i).z+" "+enteringList.get(i)+" "+interceptedList.get(i)+" "+opticalPathLength.get(i)+"\n");
            }
