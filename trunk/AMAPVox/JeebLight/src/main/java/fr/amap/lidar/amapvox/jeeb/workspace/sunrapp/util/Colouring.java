package fr.amap.lidar.amapvox.jeeb.workspace.sunrapp.util;
...
public class Colouring {
	 ...
	 */
	static public Color rainbow (float value) {
		Point3f rgb = rainbowRGB (value);
		
		if (rgb.x < 0 || rgb.x > 255) System.out.println ("Colouring: value: "+value+" red: "+rgb.x);
		if (rgb.y < 0 || rgb.y > 255) System.out.println ("Colouring: value: "+value+" green: "+rgb.y);
		if (rgb.z < 0 || rgb.z > 255) System.out.println ("Colouring: value: "+value+" blue: "+rgb.z);
	...
	/**
	 * @param value in range (0-1)
	 * @return rgb components (in range 0-255 for each) of {@link #rainbow(float)}
	 */
	static public Point3f rainbowRGB (float value) {
