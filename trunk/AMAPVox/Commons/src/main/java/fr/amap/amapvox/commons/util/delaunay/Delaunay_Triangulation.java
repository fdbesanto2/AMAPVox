package fr.amap.amapvox.commons.util.delaunay;
...
public class Delaunay_Triangulation {
	...
	private static Point_dt[] read_file(String file) throws Exception {
        ...
        public static Point_dt[] readFromArrayList(ArrayList<Vec3F> points){
            
            Point_dt[] ans = new Point_dt[points.size()];

            for (int i = 0; i < points.size(); i++) {
                
                    double d1 = points.get(i).x;
                    double d2 = points.get(i).y;
                    double d3 = 0;
                    
                    ans[i] = new Point_dt(d1, d2, d3);
                    ans[i].index = i;
            }
            return ans;
        }
