...
public class NewVoxelAnalysis extends AbstractVoxelAnalysis {
    ...
    public double computeTransmittance(Voxel voxel) {
        ...
        if ((voxel.bsEntering == 0) || (voxel.bsIntercepted > voxel.bsEntering)) {
            return Double.NaN;
        }

        // base = sum(BFOuti * Si) / sum(BFEnti * Si) 
        double base = (voxel.bsEntering - voxel.bsIntercepted) / voxel.bsEntering;
        // normalisation exponent
        // exponent = 1 / (1 / sum(BFEnti * Si) * sum(BFEnti * Si * li))
        double exponent = voxel.bsEntering / voxel.bvEntering;
